...
def refine_asset_orientation(
    ...
    instruction = (
        ...
        "Actively correct scale whenever the rendered asset is physically too large or too small: do NOT bias "
        "the estimate toward 1.0 and do NOT choose a conservative partial correction. First estimate the asset's "
        "plausible target linear size from comparable scene objects, lane width, parking-space dimensions, road "
        "geometry, and all views together. Then set scale_factor to target_linear_size / current_linear_size, "
        "using the full ratio needed to fix the mismatch. Return exactly 1.0 only when there is positive visual "
